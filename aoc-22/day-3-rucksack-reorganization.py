...
The problem is that someone forgot to put this year's updated authenticity sticker on the badges. All 
of the badges need to be pulled out of the rucksacks so the new authenticity stickers can be attached.
...
In the first group, the only item type that appears in all three rucksacks is lowercase r; this must be 
their badges. In the second group, their badge item type must be Z.
...
class Rucksack:
    ...
    def read_inputs(cls, file):
        with open(file, "r") as f:
            ...
            for line in lines.split("\n"):
                Rucksack.__inputs_list.append(line)
    

    # rucksack compartments
    def rucksack_compartments(self):
        rucksacks_compartments_list = []
        rucksacks = Rucksack.__inputs_list
        
        for rucksack in rucksacks:
            rucksacks_compartment_divide = len(rucksack) // 2
            rucksacks_compartments_list.append((rucksack[:rucksacks_compartment_divide], 
                                                rucksack[rucksacks_compartment_divide:]))
        
        return rucksacks_compartments_list


    # unique items in both compartments
    def unique_items(self):
        unique_item_list = []
        compartments = self.rucksack_compartments()
        ...
        for element in compartments:
            ...
            for i in set(temp_list):
                unique_item_list.append(i)
        
        return unique_item_list
    

    # prioritize item rearrangement
    def item_prioritization(self):
        items_dict = {}
        
        for i in range(97, 123):
            items_dict[chr(i)] = (i - 96)
        for i in range(65, 91):
            items_dict[chr(i)] = (26 + i - 64)
    ...
    def sum_priorities(self):
        ...
        for item in items:
            result += self.item_prioritization()[item]
        
        return result
    

    # sum of the priorities of 3 elfs
    def sum_priorities_three(self):
        result = 0
        count = 0
        rucksacks = Rucksack.__inputs_list
        count_range = len(rucksacks) // 3
        
        for i in range(count_range):
            temp_list = []

            for item in rucksacks[count]:
                if item in rucksacks[count + 1] and item in rucksacks[count + 2]:
                    temp_list.append(item)
            
            for i in set(temp_list):
                result += self.item_prioritization()[i]
...
elf_rucksack = Rucksack()
elf_rucksack.read_inputs("day-3-input.txt")
print(elf_rucksack.sum_priorities())
print(elf_rucksack.sum_priorities_three())